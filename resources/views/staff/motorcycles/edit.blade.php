    ...
    <div class="row g-4">
        ...
        <div class="col-lg-5">
            <div class="content-card">
                <h3 class="form-section-title">Motorcycle Image</h3>
                ...
                <div class="image-upload-section">
                    ...
                    @else
                        <div id="imagePlaceholder">
                            <i class="fas fa-motorcycle upload-placeholder"></i>
        ...
        <!-- Right Side - Motorcycle Information Form -->
        <div class="col-lg-7">
            <div class="content-card">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h3 class="form-section-title mb-0" style="border: none; padding: 0;">Motorcycle Information</h3>
                    <div class="motorcycle-id-badge">
                        <i class="fas fa-hashtag"></i>
                        ID: {{ $car->id }}
                    </div>
                </div>

                <form action="{{ route('staff.cars.update', $car->id) }}" method="POST" enctype="multipart/form-data" id="motorcycleForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            ...
                            <select name="brand" required class="form-select">
                                ...
                                <option value="Yamaha" {{ $car->brand == 'Yamaha' ? 'selected' : '' }}>Yamaha</option>
                                <option value="Kawasaki" {{ $car->brand == 'Kawasaki' ? 'selected' : '' }}>Kawasaki</option>
                                <option value="Suzuki" {{ $car->brand == 'Suzuki' ? 'selected' : '' }}>Suzuki</option>
                                <option value="Ducati" {{ $car->brand == 'Ducati' ? 'selected' : '' }}>Ducati</option>
                                <option value="BMW" {{ $car->brand == 'BMW' ? 'selected' : '' }}>BMW</option>
                                <option value="Harley-Davidson" {{ $car->brand == 'Harley-Davidson' ? 'selected' : '' }}>Harley-Davidson</option>
                                <option value="KTM" {{ $car->brand == 'KTM' ? 'selected' : '' }}>KTM</option>
                                <option value="Modenas" {{ $car->brand == 'Modenas' ? 'selected' : '' }}>Modenas</option>
                            </select>
                        </div>

                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-motorcycle text-primary"></i> Model *
                            </label>
                            <input type="text" 
                                   name="model" 
                                   value="{{ old('model', $car->model) }}"
                                   required
                                   placeholder="e.g., CBR150R, YZF-R15, Ninja"
                                   class="form-input">
                        </div>
                    </div>
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            ...
                            <select name="category" class="form-select">
                                ...
                                <option value="Cruiser" {{ $car->category == 'Cruiser' ? 'selected' : '' }}>Cruiser</option>
                                <option value="Touring" {{ $car->category == 'Touring' ? 'selected' : '' }}>Touring</option>
                                <option value="Standard" {{ $car->category == 'Standard' ? 'selected' : '' }}>Standard</option>
                                <option value="Scooter" {{ $car->category == 'Scooter' ? 'selected' : '' }}>Scooter</option>
                                <option value="Adventure" {{ $car->category == 'Adventure' ? 'selected' : '' }}>Adventure</option>
                            </select>
                        </div>
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-road text-primary"></i> Current Mileage (km)
                            </label>
                            <input type="number" 
                                   name="mileage" 
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" 
                                  rows="3" 
                                  class="form-input"
                                  placeholder="Brief description of the motorcycle...">{{ old('description', $car->description ?? '') }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="features" 
                                  rows="3" 
                                  class="form-input"
                                  placeholder="e.g., ABS, LED Lights, Digital Display, Disc Brakes">{{ old('features', $car->features ?? '') }}</textarea>
                    </div>
...
<script>
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                if (preview) {
                    ...
                    uploadSection.innerHTML = `<img src="${e.target.result}" alt="Motorcycle Preview" class="image-preview" id="imagePreview">`;
                }
    ...
    function confirmDelete() {
        if (confirm('⚠️ Are you sure you want to delete this motorcycle?\n\nThis action cannot be undone and will remove:\n• Motorcycle information\n• Associated bookings history\n• Motorcycle image\n\nType "DELETE" to confirm.')) {
